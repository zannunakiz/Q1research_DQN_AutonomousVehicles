...
def _train_dqn_run(
    ...
    Args:
        ...
        load_model_path: Path to a model whose weights should be loaded before
            training (optional). Only the DQN weights are loaded.
        start_episode: First episode number to write in logs (default: 1).
        current_stage: 1-based curriculum stage to start at. If None, inferred
            from the model filename or defaults to stage 1.
    ...
    num_stages = get_num_stages(get_obstacles())
    start_stage_idx = _resolve_start_stage(current_stage, load_model_path, num_stages)
    try:
        start_episode = max(1, int(start_episode))
    except Exception:
        start_episode = 1

    env = CarEnvironment(curriculum_stage=start_stage_idx, rulebased=bool(rulebased))

    if visualize:
        from run_loadmodel import GameRenderer, build_neuron_trace
    ...
    default_csv_name = "training_logs.csv"
    default_csv_path = os.path.join(save_dir, default_csv_name)
    if csv_log_path is None:
        csv_log_path = default_csv_path
    moving_data_csv_path = get_moving_data_csv_path(
        csv_log_path, csv_name="moving_data.csv"
    )
    ...
    episode_avg_speed_logs = []
    episode_avg_inference_time_logs = []
    moving_data_rows = []
    episode_numbers = []
    episode_q_avg_logs = []
    episode_q_max_logs = []
    ...
    if rulebased:
        print("[INFO] RULEBASED reward mode ENABLED (config_rulebased_reward.py)")
    ...
    if load_model_path:
        print(f"Loaded model (weights only): {load_model_path}")
    print("=" * 60)
    ...
    def _save_training_checkpoint(
        ...
        """Save only the DQN weights + config needed to load the model."""
        if not save_models:
            return
    ...
    collect_step_diagnostics = bool(visualize or traininfo_enabled or formula)
    ...
    while running:
        ...
        while steps < max_steps_per_episode:
            ...
            while (
                ...
                if collect_step_diagnostics or is_decision_step:
                    try:
                        q_vals_for_log = agent.get_q_values(state)
                        episode_q_values.append(q_vals_for_log)
                    except Exception:
                        pass
                ...
                total_reward += reward
                steps += 1
                if should_track_moving_frame(steps, trackMovingDataCsv):
                    append_moving_data_sample(
        ...
        episode_avg_speed_logs.append(avg_speed_str)
        episode_avg_inference_time_logs.append(avg_inference_time_str)
        ...
        moving_data_rows.append(build_moving_data_row(episode, episode_moving_track))
        ...
        episode_q_avg_logs.append(format_q_array(q_avg_vals))
        episode_q_max_logs.append(format_q_array(q_max_vals))
        episode_q_min_logs.append(format_q_array(q_min_vals))
                        ...
                        writer.writerow(
                            {
                                ...
                                "avg_speed": episode_avg_speed_logs[ep_idx],
                                "avg_inference_time": episode_avg_inference_time_logs[
                                    ep_idx
                                ],
                                "Q_avg": episode_q_avg_logs[ep_idx],
                                "Q_max": episode_q_max_logs[ep_idx],
                                "Q_min": episode_q_min_logs[ep_idx],
                            }
                        )
    ...
    try:
                ...
                writer.writerow(
                    {
                        ...
                        "avg_speed": episode_avg_speed_logs[ep_idx],
                        "avg_inference_time": episode_avg_inference_time_logs[ep_idx],
                        "Q_avg": episode_q_avg_logs[ep_idx],
                        "Q_max": episode_q_max_logs[ep_idx],
                        "Q_min": episode_q_min_logs[ep_idx],
                    }
        ...
        print(f"[OK] Final moving data CSV saved: {moving_data_csv_path}")
    except Exception as e:
...
def train_dqn(
    ...
    """Train one curriculum-learning run, optionally loading weights first."""
...
def _ensure_tester_stage_csv(
    save_dir: str, total_tester_stages: int
) -> str:
...
def run_independent_test(
    ...
    if neuron_mode and visualize:
        from run_loadmodel import build_neuron_trace
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        dest="model",
        help="Path to a .pth model whose weights to load before training (optional)",
    )
    parser.add_argument(
        "--multiply",
        action="store_true",
    ...
    if args.d3qn:
        ...
        algo = _infer_checkpoint_algo(args.model) if args.model else None
