...
from main_logger import (
    MOVING_DATA_CSV_NAME,
    append_moving_data_sample,
    build_moving_data_row,
    calculate_env_ttc_ms,
    format_avg_inference_time,
    format_avg_speed,
    format_ttc_ms,
    get_moving_data_csv_path,
    new_moving_data_track,
    save_moving_data_csv,
    should_track_moving_frame,
...
try:
    from config_obstacle import TEST_OBSTACLES, get_obstacles
except ImportError:

    def get_obstacles():  # noqa: ANN001
        return [[]]
...
def get_moving_data_csv_path_for_visualize(visualize_csv_path, log_dir=VISUALIZE_LOG_DIR):
    """Return an adaptive movingdata-<N>.csv path matching visualize-<N>.csv.

    Extracts the numeric suffix from the given visualize log filename so the
    moving-data log uses the same index. Falls back to the fixed
    ``MOVING_DATA_CSV_NAME`` when no number can be parsed.
    """
    abs_log_dir = os.path.abspath(log_dir)
    os.makedirs(abs_log_dir, exist_ok=True)
    basename = os.path.basename(visualize_csv_path)
    match = VISUALIZE_LOG_PATTERN.match(basename)
    if not match:
        return os.path.join(abs_log_dir, MOVING_DATA_CSV_NAME)
    number = int(match.group(1))
    return os.path.join(abs_log_dir, f"movingdata-{number}.csv")
...
def build_visualize_episode_row(
    ...
    row.update({
        ...
        "avg_speed": f"{float(avg_speed):.1f}",
        "avg_inference_time": f"{float(avg_inference_time):.4f}",
        "Q_avg": q_avg if q_avg is not None else "[]",
        "Q_max": q_max if q_max is not None else "[]",
        "Q_min": q_min if q_min is not None else "[]",
    })
...
class GameRenderer:
    ...
    def __init__(
        ...
        self.show_obstacle_speed_indicators = True
    ...
    def toggle_indicators(self):
        ...
        self.show_obstacle_speed_indicators = next_state
        return next_state
    ...
    def draw_obstacle_speed_indicator(self, obstacle, camera_offset):
        try:
            speed_kmh = float(obstacle.get("speed_kmh", to_kmh(obstacle.get("speed", 0.0))))
            label_text = f"{int(round(speed_kmh))} km/h"
            screen_x, screen_y = self.world_to_screen(
                float(obstacle["x"]), float(obstacle["y"]), camera_offset
            )
            if not (-100 < screen_y < self.road_display_height + 100):
                return
            ...
            label_rect.centerx = screen_x
            label_rect.bottom = int(
                screen_y - ((float(obstacle["height"]) * float(self.scale)) / 2.0) - 3
            )
            background_rect = label_rect.inflate(padding_x * 2, padding_y * 2)
            pygame.draw.rect(self.screen, (0, 0, 0), background_rect)
            pygame.draw.rect(self.screen, WHITE, background_rect, width=1)
    ...
    def draw_sensors(self, car_x, car_y, sensors, camera_offset, show_labels=False):
        ...
        for i, sensor in enumerate(sensors):
            ...
            if i == 2:
                from config_reward import OBSTACLE_WARNING_DISTANCE_FRONT
                ...
                from config_reward import OBSTACLE_WARNING_DISTANCE_SIDES
    ...
    def render(
        ...
        if self.show_obstacle_speed_indicators:
            for obstacle in info["obstacles"]:
                self.draw_obstacle_speed_indicator(obstacle, camera_offset)
...
def run_evaluation_episode(agent, env, step_multiplier=1):
    ...
    episode_ttc_samples = []
    episode_speed_samples = []
    episode_inference_time_ms = []
    episode_moving_track = new_moving_data_track()
    episode_q_values = []
    info = {
    ...
    while not done:
        for _ in range(step_multiplier):
            if step_count % DECISION_INTERVAL == 0:
                ...
                current_vis_action, action_info = agent.select_action(
                    state, training=False, debug=True
                )
                select_elapsed_ms = (time.perf_counter() - select_start) * 1000.0
                if not bool(action_info.get("explore")):
                    episode_inference_time_ms.append(float(select_elapsed_ms))
            ...
            try:
                episode_speed_samples.append(
                    float(to_kmh(info.get("speed", getattr(env, "car_speed", 0.0))))
                )
            except Exception:
                pass
            state = next_state
            step_count += 1
            if should_track_moving_frame(step_count, trackMovingDataCsv):
                append_moving_data_sample(
                    episode_moving_track, env, episode_ttc_samples
                )
    ...
    avg_speed_str = format_avg_speed(episode_speed_samples)
    avg_inference_time_str = format_avg_inference_time(episode_inference_time_ms)
    ...
    if len(episode_q_values) > 0:
        q_avg_vals = np.mean(episode_q_values, axis=0)
        q_max_vals = np.max(episode_q_values, axis=0)
        q_min_vals = np.min(episode_q_values, axis=0)
    else:
        q_avg_vals = [0.0] * agent.action_size
        q_max_vals = [0.0] * agent.action_size
        q_min_vals = [0.0] * agent.action_size
    ...
    return {
        ...
        "avg_inference_time": float(avg_inference_time_str),
        "min_sensor_logs": build_min_sensor_logs(env, episode_sensor_mins),
        "moving_data_track": episode_moving_track,
        "final_info": info,
        "q_avg": format_q_array(q_avg_vals),
        "q_max": format_q_array(q_max_vals),
        "q_min": format_q_array(q_min_vals),
    }
...
def run_evamodel(models_dir="models", algo=ALGO_DQN, log_dir=VISUALIZE_LOG_DIR):
    ...
    moving_data_csv_path = get_moving_data_csv_path(evaluate_csv_path)
    ...
    env = CarEnvironment(obstacles_config=get_obstacles())
    agent = None
    agent_algo = None
    visualize_rewards = []
    moving_data_rows = []
    ...
    with open(evaluate_csv_path, "w", newline="", encoding="utf-8") as csv_file:
        ...
        for model_index, (model_number, model_path) in enumerate(model_entries, start=1):
            ...
            row = build_visualize_episode_row(
                ...
                q_avg=metrics.get("q_avg"),
                q_max=metrics.get("q_max"),
                q_min=metrics.get("q_min"),
            )
            csv_writer.writerow(row)
            csv_file.flush()
            moving_data_rows.append(
                build_moving_data_row(
                    rows_written, metrics.get("moving_data_track", {})
                )
            )
            save_moving_data_csv(moving_data_csv_path, moving_data_rows)
            ...
            print(f"  Avg Speed: {float(metrics['avg_speed']):.1f} km/h")
            print(
                f"  Avg Inference Time: {float(metrics['avg_inference_time']):.2f} ms"
            )
            ...
            print(
                ...
                f"avg_speed={float(metrics['avg_speed']):.1f}, "
                f"avg_inference_time={float(metrics['avg_inference_time']):.2f}"
            )
...
def run_visualization(
    ...
    if experiment_mode:
        ...
        obstacles_cfg = TEST_OBSTACLES if tester else get_obstacles()

    if experiment_mode:
        env = CarEnvironment(obstacles_config=obstacles_cfg, disable_finish=True, rulebased=bool(rulebased))
    elif random_mode:
        env = CarEnvironment(obstacles_config=obstacles_cfg, disable_finish=False, rulebased=bool(rulebased))
    elif allstage:
        env = CarEnvironment(curriculum_stage=0, obstacles_config=obstacles_cfg, rulebased=bool(rulebased))
    else:
        env = CarEnvironment(obstacles_config=obstacles_cfg, rulebased=bool(rulebased))
    ...
    moving_data_csv_path = get_moving_data_csv_path_for_visualize(visualize_csv_path)
    visualize_rewards = []
    print(f"Visualize CSV: {visualize_csv_path}")
    print(f"Moving data CSV: {moving_data_csv_path}")
    ...
    def save_moving_csv(current_track=None):
        try:
            current_row = (
                build_moving_data_row(episode, current_track)
                if current_track is not None
                else None
            )
            save_moving_data_csv(
                moving_data_csv_path, completed_moving_data_rows, current_row
            )
        except Exception as e:
            print(f"Error saving moving data CSV: {e}")
    ...
    while running and (experiment_mode or episode < episodes):
        ...
        while running and not done:
            ...
            for _ in range(steps_this_frame):
                if manual_mode:
                    ...
                    if step_count % DECISION_INTERVAL == 0:
                        ...
                        if not bool(action_info.get("explore")):
                            episode_inference_time_ms.append(
                                float(select_elapsed_ms)
                            )
            ...
            if not paused:
                ...
                if now_ticks - last_log_ticks >= visualize_logs_sec * 1000:
                    ...
                    save_moving_csv(episode_moving_track)
        ...
        if done:
            ...
            ttc_summary = summarize_ttc_samples(episode_ttc_samples)
            avg_speed_str = format_avg_speed(episode_speed_samples)
            avg_inference_time_str = format_avg_inference_time(
                episode_inference_time_ms
            )
            ...
            completed_moving_data_rows.append(
                build_moving_data_row(episode, episode_moving_track)
            )
            save_visualize_csv()
            save_moving_csv()
            ...
            print(f"  Avg Speed: {avg_speed_str} km/h")
            print(f"  Avg Inference Time: {avg_inference_time_str} ms")
            ...
            print(
                ...
                f"avg_speed={avg_speed_str}, avg_inference_time={avg_inference_time_str}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tester",
        action="store_true",
        help="Use TEST_OBSTACLES from config_obstacle.py instead of OBSTACLES",
    )
    parser.add_argument(
        "--experiment",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use rule-based reward system (config_rulebased_reward.py) instead of the "
            "standard DQN reward (config_reward.py). Useful for research baseline comparison."
        ),
    )
