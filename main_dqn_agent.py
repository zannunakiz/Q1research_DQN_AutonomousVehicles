...
def _align_tensor_to_module_device(module: nn.Module, x):
...
class DQNNetwork(nn.Module):
    ...
    def forward(self, x):
        x = _align_tensor_to_module_device(self, x)
...
class DQNAgent:
    ...
    def save(self, filepath):
        ...
        Only the DQN weights (policy + target networks) and the network
        configuration required to reconstruct and load the model are stored.
        Training-only runtime state (replay buffer, optimizer, epsilon, loss
        history, update counter, curriculum/training state) is intentionally
        NOT saved -- a fresh load does not need any of it.

        Uses legacy (non-zip) torch serialization so the model filename is not
        embedded inside the artifact, keeping the file anonymous w.r.t. its name.
        """
        payload = {
        ...
        try:
            # Non-zip serialization -> no filename clue inside the file.
            torch.save(payload, filepath, _use_new_zipfile_serialization=False)
        except TypeError:
            torch.save(payload, filepath)
        print(f"Model saved to {filepath}")

    def load(self, filepath, checkpoint=None, load_replay_buffer=True):
        """Load model weights and any available training state.

        Only the policy and target networks are required. Every other field
        (optimizer, replay buffer, epsilon, loss history, update counter) is
        restored only when the saved artifact actually contains it, so plain
        weight-only files still load cleanly.
        """
        ...
        if not isinstance(checkpoint, dict):
            raise ValueError(
                f"Unsupported model artifact at {filepath}. Expected a dict "
                "containing 'policy_net_state_dict'."
            )
        ...
        if isinstance(config, dict):
            ...
            saved_algo = str(config.get('algo', self.algo)).strip().lower()
            if saved_algo in SUPPORTED_ALGOS:
                self.algo = saved_algo
        ...
        if 'algo' in checkpoint:
            restored_algo = str(checkpoint['algo']).strip().lower()
            if restored_algo in SUPPORTED_ALGOS:
                self.algo = restored_algo
        if isinstance(checkpoint.get('loss_history'), list):
            self.loss_history = checkpoint['loss_history']
        if 'update_counter' in checkpoint:
            try:
                self.update_counter = int(checkpoint['update_counter'])
            except Exception:
                pass
        if (
            load_replay_buffer
            and checkpoint.get('replay_buffer') is not None
        ):
            self.memory = ReplayBuffer(capacity=self.memory_size)
