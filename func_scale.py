...
from config_simulation import DEFAULT_SCALE, FPS

PX_PER_METER = 14.0

def meter_to_pygame(meter_value: float, default_scale: float = 1.5) -> int:
    """Convert real-world meters to Pygame world units.

    20 meters to 185 units as proper scale base and rounds all other conversions
    ...
    pygame_value = pixel_value / float(default_scale)

    return round(pygame_value)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scale helper CLI for config_simulation.py values using 14 px = 1 meter."
    )
    parser.add_argument(
        "--tometer",
        type=float,
