...
import math
import random
from main_logger import calculate_env_ttc_details
from func_scale import to_kmh, to_pygame_speed
from config_simulation import (
    ...
    OBSTACLE_HEIGHT,
    OBSTACLE_SPEED,
    OBSTACLE_ADAPTIVE_SPEED,
    OBSTACLE_MAX_SPEED,
    OBSTACLE_MIN_SPEED,
    OBSTACLE_STEP_SPEED,
    OBSTACLE_INTERVAL_SPEED,
    OBSTACLE_SPEED_GRADUAL,
...
from config_rulebased_reward import (
    REWARD_MOVETO_CLEARSIDE,
    REWARD_SLOW_WHEN_DETECT,
    REWARD_FAST_WHEN_CLEAR as RB_REWARD_FAST_WHEN_CLEAR,
    REWARD_FINISH as RB_REWARD_FINISH,
    PENALTY_MOVETO_UNCLEARSIDE,
    PENALTY_COLLISION as RB_PENALTY_COLLISION,
    RULEBASED_FRONT_CLEAR_THRESHOLD,
)
from config_obstacle import get_obstacles
...
def get_flat_obstacles(obstacles_config, stage=None):
    ...
    Args:
        obstacles_config: The OBSTACLES configuration from config_obstacle.py
...
class CarEnvironment:
    def __init__(
        ...
        self.obstacles_config = (
            obstacles_config if obstacles_config is not None else get_obstacles()
        )
        ...
        self.obstacle_min_speed, self.obstacle_max_speed = (
            self._normalize_obstacle_speed_bounds(
                OBSTACLE_MIN_SPEED, OBSTACLE_MAX_SPEED
            )
        )
        self.obstacle_step_speed = max(0.0, float(to_pygame_speed(OBSTACLE_STEP_SPEED)))
        self.obstacle_interval_speed = max(1, int(OBSTACLE_INTERVAL_SPEED))
        self.obstacle_speed_gradual = max(1, int(OBSTACLE_SPEED_GRADUAL))
        self.obstacle_adaptive_speed = bool(OBSTACLE_ADAPTIVE_SPEED)
        self.obstacle_speed = self._resolve_obstacle_initial_speed(
            OBSTACLE_SPEED, self.obstacle_min_speed, self.obstacle_max_speed
        )
    ...
    @staticmethod
    def _normalize_obstacle_speed_bounds(min_speed, max_speed):
        try:
            first_speed = float(min_speed)
            second_speed = float(max_speed)
        except (TypeError, ValueError):
            return 0.0, 0.0
        if not math.isfinite(first_speed) or not math.isfinite(second_speed):
            return 0.0, 0.0
        lower_speed = min(first_speed, second_speed)
        upper_speed = max(first_speed, second_speed)
        return max(0.0, lower_speed), max(0.0, upper_speed)

    def _resolve_obstacle_initial_speed(self, speed_value, min_speed, max_speed):
        min_speed, max_speed = self._normalize_obstacle_speed_bounds(min_speed, max_speed)
        try:
            resolved_speed = float(speed_value)
        except (TypeError, ValueError):
            return min_speed
        if not math.isfinite(resolved_speed):
            return min_speed
        if resolved_speed < min_speed or resolved_speed > max_speed:
            return min_speed
        return resolved_speed

    def _parse_obstacle_adaptive_bounds(self, adaptive_value):
        if adaptive_value is None:
            return None
        values = None
        if isinstance(adaptive_value, str):
            parts = adaptive_value.strip().replace(" ", "").split("-")
            if len(parts) == 2:
                values = parts
        elif isinstance(adaptive_value, dict):
            values = [
                adaptive_value.get("min", adaptive_value.get("from")),
                adaptive_value.get("max", adaptive_value.get("to")),
            ]
        else:
            try:
                if len(adaptive_value) == 2:
                    values = list(adaptive_value)
            except TypeError:
                values = None
        if values is None:
            return None
        try:
            min_kmh = float(values[0])
            max_kmh = float(values[1])
        except (TypeError, ValueError):
            return None
        if not math.isfinite(min_kmh) or not math.isfinite(max_kmh):
            return None
        return self._normalize_obstacle_speed_bounds(
            to_pygame_speed(min_kmh), to_pygame_speed(max_kmh)
        )

    def _resolve_obstacle_adaptive_bounds(self, obs_config):
        global_min = float(self.obstacle_min_speed)
        global_max = float(self.obstacle_max_speed)
        if "adaptive" in obs_config:
            parsed_bounds = self._parse_obstacle_adaptive_bounds(obs_config.get("adaptive"))
            if parsed_bounds is None:
                parsed_bounds = (global_min, global_max)
            adaptive_min, adaptive_max = parsed_bounds
            adaptive_min = self._clamp(adaptive_min, global_min, global_max)
            adaptive_max = self._clamp(adaptive_max, global_min, global_max)
            return self._normalize_obstacle_speed_bounds(adaptive_min, adaptive_max)
        if self.obstacle_adaptive_speed:
            return global_min, global_max
        return None

    def _update_obstacle_adaptive_speed(self, obs):
        if not obs.get("adaptive_speed_enabled"):
            return

        interval = max(1, int(obs.get("adaptive_interval", self.obstacle_interval_speed)))
        remaining = int(obs.get("adaptive_interval_remaining", interval)) - 1

        if remaining <= 0:
            min_speed, max_speed = self._normalize_obstacle_speed_bounds(
                obs.get("adaptive_min_speed", self.obstacle_min_speed),
                obs.get("adaptive_max_speed", self.obstacle_max_speed),
            )
            step_speed = max(0.0, float(obs.get("adaptive_step_speed", self.obstacle_step_speed)))
            
            target_speed = float(obs.get("target_speed", obs.get("speed", min_speed)))
            current_target = self._clamp(target_speed, min_speed, max_speed)
            
            choices = ["stay"]
            if step_speed > 0.0 and current_target < max_speed - 1e-12:
                choices.append("increase")
            if step_speed > 0.0 and current_target > min_speed + 1e-12:
                choices.append("decrease")

            decision = random.choice(choices)
            if decision == "increase":
                current_target = self._clamp(current_target + step_speed, min_speed, max_speed)
            elif decision == "decrease":
                current_target = self._clamp(current_target - step_speed, min_speed, max_speed)

            obs["target_speed"] = current_target
            obs["speed_gradual_remaining"] = max(1, int(obs.get("adaptive_speed_gradual", self.obstacle_speed_gradual)))
            remaining = interval
            
        obs["adaptive_interval_remaining"] = remaining

        # Process gradual speed transition
        gradual_remaining = obs.get("speed_gradual_remaining", 0)
        if gradual_remaining > 0:
            target_speed = float(obs.get("target_speed", obs.get("speed", 0.0)))
            current_speed = float(obs.get("speed", 0.0))
            speed_error = target_speed - current_speed
            speed_delta = speed_error / float(gradual_remaining)
            new_speed = current_speed + speed_delta
            obs["speed"] = new_speed
            obs["speed_kmh"] = float(to_kmh(new_speed))
            obs["speed_gradual_remaining"] = gradual_remaining - 1
        else:
            target_speed = float(obs.get("target_speed", obs.get("speed", 0.0)))
            obs["speed"] = target_speed
            obs["speed_kmh"] = float(to_kmh(target_speed))
    ...
    def _resolve_obstacle_speed(self, speed_value):
        ...
        max_obstacle_speed = max(0.0, float(getattr(self, "obstacle_max_speed", self.max_speed)))
    ...
    def _build_obstacle_state(self, obs_config):
        ...
        try:
            ...
            x_pos = self._lane_center_x(lane)
        fixed_speed = "speed" in obs_config and obs_config.get("speed") is not None
        adaptive_bounds = None if fixed_speed else self._resolve_obstacle_adaptive_bounds(obs_config)
        obstacle_speed = self._resolve_obstacle_speed(obs_config.get("speed"))
        if adaptive_bounds is not None:
            obstacle_speed = self._resolve_obstacle_initial_speed(
                obstacle_speed, adaptive_bounds[0], adaptive_bounds[1]
            )
        ...
        obstacle = {
            ...
            "speed_kmh": float(to_kmh(obstacle_speed)),
        }
        if adaptive_bounds is not None:
            obstacle.update(
                {
                    "adaptive_speed_enabled": True,
                    "adaptive_min_speed": adaptive_bounds[0],
                    "adaptive_max_speed": adaptive_bounds[1],
                    "adaptive_step_speed": self.obstacle_step_speed,
                    "adaptive_interval": self.obstacle_interval_speed,
                    "adaptive_interval_remaining": self.obstacle_interval_speed,
                    "adaptive_speed_gradual": self.obstacle_speed_gradual,
                    "speed_gradual_remaining": 0,
            ...
            self.initial_obstacles.append(
                {
                    ...
                    "adaptive": obs_config.get("adaptive"),
    ...
    def step(self, action, apply_steering=True):
        ...
        for obs in self.obstacles:
            self._update_obstacle_adaptive_speed(obs)
            if obs.get("adaptive_speed_enabled"):
                obstacle_speed = float(obs.get("speed", 0.0))
            else:
            ...
            if obstacle_speed > 0:
                ...
                obs["speed_kmh"] = float(to_kmh(obstacle_speed))
        ...
        if self.rulebased:
            reward = self._calculate_rulebased_reward()
        else:
    ...
    def _calculate_rulebased_reward(self):
        """Rule-based reward for baseline comparison (used when rulebased=True).

        Sensor indices: [R2=0, R1=1, F=2, L1=3, L2=4, SR=5, SL=6]
        Action space  : 0=slow_left 1=slow_straight 2=slow_right
                        3=fast_left 4=fast_straight  5=fast_right

        Rules applied (in order of priority):
          1. PENALTY_COLLISION  if collided with obstacle or wall
          2. REWARD_FINISH      if finish line reached
          3. REWARD_MOVETO_CLEARSIDE / PENALTY_MOVETO_UNCLEARSIDE
             Compare mean normalized distance of left sensors (SL, L1, L2)
             vs right sensors (SR, R1, R2). If left is safer and action turns
             left (0 or 3) -> reward; if right is safer and action turns right
             (2 or 5) -> reward. Opposing choices receive penalty.
             Straight actions (1, 4) are neutral for this rule.
          4. REWARD_SLOW_WHEN_DETECT  if front sensor detects obstacle AND
             action is a slow class (0, 1, 2).
          5. RB_REWARD_FAST_WHEN_CLEAR if front is clear AND action == 4
             (fast straight).
        """
        reward = 0.0
        self.warning_front = False
        ...
        # --- Rule 1: Collision penalty (highest priority, exit immediately) ---
        if self._check_collision_with_obstacles():
            reward += RB_PENALTY_COLLISION
            return reward
        ...
        if (
            ...
            self.reached_finish = True
            reward += RB_REWARD_FINISH
            return reward

        # Read normalized sensor values (0=blocked, 1=fully clear)
        # Sensor order: [R2, R1, F, L1, L2, SR, SL]
        sensor_readings = []
        for idx in range(self.num_sensors):
            measured = self._measure_sensor(idx, apply_noise=True)
            sensor_readings.append(float(measured["normalized"]))

        norm_r2, norm_r1, norm_f, norm_l1, norm_l2, norm_sr, norm_sl = sensor_readings

        # --- Rule 3: Move toward clear side ---
        # Left side safety = mean of SL, L1, L2 (higher = more space on the left)
        left_safety  = (norm_sl + norm_l1 + norm_l2) / 3.0
        # Right side safety = mean of SR, R1, R2 (higher = more space on the right)
        right_safety = (norm_sr + norm_r1 + norm_r2) / 3.0

        action = self.last_action
        is_left_action  = action in (0, 3)   # slow_left or fast_left
        is_right_action = action in (2, 5)   # slow_right or fast_right

        if abs(left_safety - right_safety) > 0.05:  # only apply when sides differ meaningfully
            if left_safety > right_safety:
                # Left side is safer
                if is_left_action:
                    reward += REWARD_MOVETO_CLEARSIDE
                elif is_right_action:
                    reward += PENALTY_MOVETO_UNCLEARSIDE
            else:
                # Right side is safer
                if is_right_action:
                    reward += REWARD_MOVETO_CLEARSIDE
        ...
        # --- Rule 4: Slow when front detects obstacle ---
        front_clear = norm_f >= RULEBASED_FRONT_CLEAR_THRESHOLD
        if not front_clear:
            self.warning_front = True
            self.warning_close_count_step += 1
            if action in (0, 1, 2):  # any slow action
                reward += REWARD_SLOW_WHEN_DETECT

        # --- Rule 5: Fast straight when front is clear ---
        if front_clear and action == 4:  # fast_straight
            reward += RB_REWARD_FAST_WHEN_CLEAR
